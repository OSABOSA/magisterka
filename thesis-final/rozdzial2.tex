...
W rozdziale tym omówiono technologie wykorzystane w projekcie badawczym, które nie zostały szczegółowo opisane w części teoretycznej (Rozdział~1). Nacisk położono na komponenty specyficzne dla środowiska Google Cloud Platform, czyli GCP: zarządzany klaster GKE, równoważenie obciążenia przez GCP LoadBalancer, maszynę Compute Engine jako generator obciążenia testowego oraz stos monitorujący kube-prometheus-stack z prometheus-adapter jako mostkiem między metrykami aplikacyjnymi a API HPA.

\section{Google Kubernetes Engine — zarządzany Kubernetes w GCP}

Google Kubernetes Engine, czyli GKE, jest zarządzaną usługą Kubernetes oferowaną przez Google Cloud Platform, certyfikowaną przez CNCF pod kątem pełnej zgodności API \cite{gke-docs}. W odróżnieniu od samodzielnie administrowanego klastra, GKE automatycznie zarządza warstwą Control Plane; użytkownik nie ma bezpośredniego dostępu do komponentów takich jak etcd czy kube-apiserver, co eliminuje konieczność ich ręcznej konfiguracji i utrzymania.
...
kube-prometheus-stack jest pakietem Helm, czyli Helm chart,~\cite{helm-docs} rozwijanym przez społeczność prometheus-community, który instaluje kompletny stos monitorujący dla Kubernetes \cite{kube-prometheus-stack}. W skład pakietu wchodzą: Prometheus (system zbierania i przechowywania metryk w postaci szeregów czasowych), Grafana (platforma wizualizacji danych), kube-state-metrics (komponent generujący metryki o stanie obiektów Kubernetes), node-exporter (komponent zbierający metryki systemowe z węzłów) oraz Prometheus Operator (kontroler zarządzający cyklem życia instancji Prometheusa) \cite{prometheus-operator}.
...
Prometheus jest otwartoźródłowym systemem monitorowania, pierwotnie opracowanym w SoundCloud, obecnie inkubowanym przez CNCF \cite{prometheus2018}. Jego architektura opiera się na modelu pull: Prometheus aktywnie pobiera, czyli scrapuje, metryki z aplikacji poprzez HTTP w konfigurowalnych interwałach, w projekcie co 15 sekund. Zebrane dane są przechowywane w natywnej bazie szeregów czasowych (TSDB) z etykietami umożliwiającymi filtrowanie i agregację. Język zapytań PromQL pozwala na zaawansowane operacje: agregacje, operacje na zakresach czasowych oraz predykcję.
...
Dla każdego scenariusza testowego z Grafany eksportowanych jest 5 zestawów danych CSV: liczba replik w czasie, zużycie CPU per Pod w mCPU, zużycie pamięci per Pod w MiB, przepustowość w RPS oraz percentyle opóźnień w ms. Dane te, wraz z wynikami k6, stanowią kompletny zbiór danych badawczych dla analizy w Rozdziale~5.

\subsection{Prometheus Adapter}

Prometheus Adapter (wersja v0.12.0) jest komponentem implementującym Kubernetes Custom Metrics API \cite{prometheus-adapter}. Jego rola polega na rejestracji jako APIService w Kubernetes (rozszerzając API o ścieżki \texttt{/apis/custom.metrics.k8s.io/}), pobieraniu metryk z Prometheusa poprzez zapytania PromQL oraz serwowaniu tych metryk w formacie oczekiwanym przez HPA.
...
\begin{lstlisting}[language=yaml, caption={Konfiguracja Prometheus Adapter — wybrane reguły mapowania metryk}, label={lst:adapter-config}]
rules:
  - seriesQuery: 'cpu_service_http_requests_total{namespace!="",pod!=""}'
    ...
    metricsQuery: 'sum(rate(cpu_service_http_requests_total{<<.LabelMatchers>>}[1m])) by (<<.GroupBy>>)'
  - seriesQuery: 'io_service_http_requests_total{namespace!="",pod!=""}'
    resources:
      overrides:
        namespace: {resource: "namespace"}
        pod: {resource: "pod"}
    name:
      matches: "io_service_http_requests_total"
      as: "http_requests_per_second"
    metricsQuery: 'sum(rate(io_service_http_requests_total{<<.LabelMatchers>>}[1m])) by (<<.GroupBy>>)'
  # ... (pozostałe dwie reguły dla metryk in-flight, analogiczne)
\end{lstlisting}

\section{k6 — narzędzie do testów obciążeniowych}

k6 (Grafana k6) jest otwartoźródłowym narzędziem do testów obciążeniowych, napisanym w Go z interfejsem skryptowym w JavaScript (ES6) \cite{k6-docs}. Pojedyncza instancja k6 jest w stanie generować setki wirtualnych użytkowników, czyli VU (Virtual Users) bez potrzeby architektury rozproszonej, co czyni ją odpowiednią do testów średniej skali bez dodatkowej infrastruktury.

W projekcie zdefiniowano trzy skrypty k6 odpowiadające profilom obciążeniowym i scenariuszom badawczym. Skrypt \texttt{cpu-load-test.js} zawiera dwa scenariusze: Fibonacci (ramp-up od 1 do 50 VU przez 60 sekund, następnie stałe obciążenie 50 VU przez 120 sekund) oraz przetwarzanie obrazu (stałe 10 VU wysyłających losowo generowane obrazy PNG do endpointu \texttt{POST /process}. Skrypt \texttt{io-load-test.js} zawiera trzy scenariusze: zapytania ze stałym opóźnieniem z ramp-up od 1 do 100 VU, test obciążeniowy z rosnącym opóźnieniem od 50 do 2000\,ms oraz wywołania zewnętrzne. Skrypt \texttt{combined-scenario.js} uruchamia trzy scenariusze równolegle: Fibonacci (30 VU), przetwarzanie obrazu (5 VU) oraz zapytania IO (80 VU).

Każdy skrypt definiuje progi akceptacji, czyli thresholds: percentyl p95 czasu odpowiedzi poniżej 5000\,ms dla scenariuszy CPU oraz poniżej 10000\,ms dla scenariuszy IO, a także dopuszczalną stopę błędów HTTP ($<1\%$ dla CPU, $<5\%$ dla IO). Przekroczenie tych progów oznacza niepowodzenie testu.

\section{Docker i konteneryzacja}

Każdy mikroserwis w projekcie jest konteneryzowany za pomocą Docker \cite{docker2023}. Wszystkie Dockerfile opierają się na wspólnym wzorcu: obraz bazowy \texttt{python:3.12-slim}, wybrany ze względu na minimalny rozmiar; instalacja zależności przez \texttt{pip install --no-cache-dir}; aplikacja jako pojedynczy plik \texttt{main.py}; uruchomienie przez Uvicorn z parametrami \texttt{--host 0.0.0.0 --port 8000}.

Obrazy są budowane lokalnie i przesyłane do GCP Artifact Registry \cite{gcp-artifact-registry}, zarządzanego repozytorium obrazów kontenerowych Google Cloud, pełniącego w projekcie rolę rejestru obrazów dla GKE. W przeciwieństwie do publicznego Docker Hub, Artifact Registry zapewnia niskie opóźnienie pobierania obrazów w ramach tej samej sieci VPC, kontrolę dostępu przez IAM oraz automatyczne skanowanie podatności. GKE pobiera obrazy automatycznie podczas wdrażania na podstawie pola \texttt{image} w Deployment. W projekcie nie wykorzystuje się Docker Compose do orkiestracji testów; wszystkie eksperymenty są przeprowadzane bezpośrednio na klastrze GKE, co zapewnia zgodność środowiska testowego z produkcyjnym. Plik \texttt{docker-compose.yaml} znajdujący się w repozytorium służy wyłącznie do opcjonalnego, szybkiego uruchomienia serwisów w środowisku deweloperskim.
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{|l|X|X|}
    ...
    \hline
    Język / Framework & Python 3.12 + FastAPI \cite{fastapi} & Implementacja logiki mikroserwisów: cpu-service, io-service, echo-server, memory-service \\
    Serwer ASGI & Uvicorn & Obsługa żądań HTTP z wykorzystaniem pętli zdarzeń asyncio \\
    Konteneryzacja & Docker \cite{docker2023} & Budowa lekkich obrazów kontenerowych na bazie \texttt{python:3.12-slim} \\
    Rejestr obrazów & GCP Artifact Registry \cite{gcp-artifact-registry} & Prywatny rejestr obrazów kontenerowych w sieci VPC; źródło dla GKE \\
    Orkiestracja & GKE (Google Kubernetes Engine) \cite{gke-docs} & Zarządzanie klastrem, harmonogramowanie Podów, \textit{cluster autoscaler} \\
    Sieć & GCP VPC + LoadBalancer \cite{gcp-lb-docs} & Komunikacja wewnętrzna i ekspozycja serwisów na zewnętrzne adresy IP \\
    Monitoring & kube-prometheus-stack \cite{kube-prometheus-stack} (Helm \cite{helm-docs}) & Zbieranie metryk (Prometheus), wizualizacja (Grafana), metryki stanu klastra (kube-state-metrics) \\
    Custom Metrics & prometheus-adapter v0.12.0 \cite{prometheus-adapter} & Translacja zapytań PromQL na Custom Metrics API Kubernetes dla HPA \\
    Testy obciążeniowe & k6 v0.48+ \cite{k6-docs} & Generowanie ruchu HTTP i pomiar metryk wydajnościowych (RPS, percentyle opóźnień) \\
    Generator obciążenia & GCP Compute Engine \cite{gce-docs} & Dedykowana maszyna wirtualna (\texttt{k6-generator}) izolująca obciążenie testowe od klastra \\
    \hline
  \end{tabularx}
  \caption{Podsumowanie stosu technologicznego projektu wraz z rolami poszczególnych komponentów w systemie badawczym}
