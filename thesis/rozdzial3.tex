...
\begin{itemize}
  ...
  \item \texttt{GET /fibonacci} — obliczanie $n$-tej liczby Fibonacciego rekurencyjnym algorytmem o złożoności $O(2^{n})$. Parametr $n$ jest konfigurowalny (domyślnie $n = 30$).
\end{itemize}
...
\begin{lstlisting}[language=yaml, caption={Konfiguracja HPA CPU dla \texttt{cpu-service} (plik \texttt{k8s/cpu-service/hpa-cpu.yaml})}, label={lst:hpa-cpu-config}]
metrics:
  - type: Resource
    resource:
      name: cpu
...
\begin{lstlisting}[language=yaml, caption={Konfiguracja HPA Memory dla \texttt{io-service} (plik \texttt{k8s/io-service/hpa-memory.yaml})}, label={lst:hpa-memory-config}]
metrics:
  - type: Resource
    resource:
      name: memory
...
\begin{lstlisting}[language=yaml, caption={Konfiguracja HPA Custom RPS (plik \texttt{k8s/cpu-service/hpa-custom.yaml})}, label={lst:hpa-custom-config}]
metrics:
  - type: Pods
    pods:
      metric:
...
\textit{Prometheus} pełni rolę centralnego systemu zbierania metryk. Jest skonfigurowany do scrapowania:

\begin{itemize}
  \item \textbf{Metryk infrastrukturalnych} — z \texttt{kubernetes-sd} (CPU, RAM, liczba replik, stan Podów) co 15 sekund.
  \item \textbf{Metryk aplikacyjnych} — z endpointów \texttt{/metrics} wystawianych przez \texttt{cpu-service}, \texttt{io-service} i \texttt{echo-server} (RPS, czas przetwarzania, liczba aktywnych zapytań, metryki niestandardowe).
\end{itemize}

\subsection{Grafana}

\textit{Grafana} udostępnia dashboard ,,Magisterka — System Overview'' zawierający 16 paneli wizualizujących kluczowe metryki systemu w czasie rzeczywistym (odświeżanie co 10 sekund):
...
\begin{itemize}
  ...
  \item \textbf{3 Serwisy} typu \texttt{ClusterIP}: każdy kieruje ruch na port 80 do \texttt{targetPort 8000} odpowiedniego \textit{Deploymentu}.
  \item \textbf{6 HPA} (po 3 na serwis badawczy): pliki \texttt{hpa-cpu.yaml}, \texttt{hpa-memory.yaml}, \texttt{hpa-custom.yaml}.
  \item \textbf{Monitoring}: \texttt{prometheus} + \texttt{grafana} + \texttt{prometheus-adapter} w tym samym namespace.
  \item \textbf{Ingress} (\textit{Traefik}): routing na podstawie nazwy hosta (\texttt{cpu-magisterka.k3s.local} $\rightarrow$ \texttt{cpu-service}, \texttt{io-magisterka.k3s.local} $\rightarrow$ \texttt{io-service}).
\end{itemize}

W środowisku testowym ruch z \textit{k6} do serwisów jest kierowany przez \texttt{kubectl port-forward}, co eliminuje potrzebę zewnętrznego \textit{LoadBalancera} i zapewnia minimalne opóźnienie sieciowe ($<1$\,ms, lokalny \texttt{localhost}).
...
Każdy scenariusz jest wykonywany według ściśle określonej procedury, zaimplementowanej w skrypcie \texttt{scripts/run-tests.sh}:

\begin{enumerate}
  \item \textbf{Przełączenie HPA} — usunięcie wszystkich istniejących HPA dla docelowego serwisu i nałożenie pliku HPA odpowiadającego testowanej strategii.
  \item \textbf{Reset Deploymentu} — skalowanie \textit{Deploymentu} do 1 repliki i oczekiwanie na gotowość (\texttt{rollout status}).
  \item \textbf{Uruchomienie testu k6} — wykonanie właściwego skryptu obciążeniowego.
  \item \textbf{Zapis wyników} — wyniki k6 (JSON + tekstowe podsumowanie) zapisywane do katalogu \texttt{results/<scenariusz>/}.
...
\begin{itemize}
  ...
  \item \textbf{Scenariusz przetwarzanie obrazu}: stałe 10 VU przez 180 sekund, każde żądanie wysyła losowo wygenerowany obraz PNG ($100 \times 100$ pikseli) do \texttt{POST /process}.
\end{itemize}

\textbf{I/O-bound} (skrypt \texttt{io-load-test.js}):
\begin{itemize}
  \item \textbf{Scenariusz stałych zapytań}: ramp-up od 1 do 100 VU w ciągu 60 sekund, stałe obciążenie przez 120 sekund, \texttt{delay=100\,ms}, \texttt{steps=3}.
  \item \textbf{Scenariusz stres-test}: stałe 20 VU, \texttt{delay} rosnący od 50\,ms do 2000\,ms w krokach co 30 sekund.
  \item \textbf{Scenariusz wywołań zewnętrznych}: stałe 20 VU, \texttt{external\_call=true}, żądania do \texttt{GET /upstream}.
\end{itemize}
...
\begin{itemize}
  ...
  \item \textbf{Liczba replik w czasie} — przebieg czasowy skalowania (dane z \textit{Prometheusa}).
  \item \textbf{Zużycie CPU i pamięci} — średnie i szczytowe zużycie zasobów na Pod.
\end{itemize}

\section{Podsumowanie architektury}

Zaprojektowany system badawczy umożliwia kontrolowane, powtarzalne eksperymenty porównujące skuteczność trzech strategii HPA dla dwóch profili obciążeniowych. Kluczowe decyzje architektoniczne:

\begin{enumerate}
  \item \textbf{Izolacja zmiennych} — każdy scenariusz testuje dokładnie jedną kombinację (profil $\times$ strategia), przy wyizolowanym serwisie (port-forward) i ustabilizowanym klastrze (cooldown).
  ...
  \item \textbf{Automatyzacja} — cały proces (przełączanie HPA, reset Deploymentów, uruchamianie k6, zbieranie wyników) jest w pełni zautomatyzowany przez skrypt \texttt{run-tests.sh}.
  \item \textbf{Odtwarzalność} — wszystkie manifesty \textit{Kubernetes}, skrypty k6 i konfiguracje są przechowywane w repozytorium, co umożliwia odtworzenie środowiska na dowolnym klastrze \textit{Kubernetes}.
\end{enumerate}
